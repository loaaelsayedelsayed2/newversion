                                ...
                                <form action="{{route('admin.business-settings.set-provider-setup')}}"
                                    ...
                                    <div class="row g-4">
                                        ...
                                        <div class="col-md-12 col-12 mb-30">
                                            <div class="border p-3 rounded d-flex justify-content-between">
                                                <div>
                                                    <div class="">{{translate('Service at Provider Place')}}</div>
                                                    <div>{{translate($serviceAtProviderPlace[0]['info_message'] ?? '')}}</div>
                                                </div>
                                                <label class="switcher">
                                                    @php($value = $dataValues->where('key_name', $serviceAtProviderPlace[0]['key'])?->first()?->live_values ?? null)
                                                    <input class="switcher_input {{ $value ? '' : 'switch_alert' }}"
                                                           id="{{$serviceAtProviderPlace[0]['key']}}"
                                                           type="checkbox"
                                                           name="{{$serviceAtProviderPlace[0]['key']}}"
                                                           value="1" {{$value ? 'checked' : ''}}
                                                           data-id="{{$serviceAtProviderPlace[0]['key']}}"
                                                           data-message="{{ucfirst(translate($serviceAtProviderPlace[0]['key']))}}">
                                                    <span class="switcher_control"></span>
                                                </label>
                                            </div>
                                        </div>
