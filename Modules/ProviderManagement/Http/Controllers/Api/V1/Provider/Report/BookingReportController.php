...
class BookingReportController extends Controller
{
    ...
    public function getBookingReport(Request $request)
    {
        ...
        $filteredBookings->getCollection()->transform(function ($booking) {
            if ($booking->customer === null && isset($booking->service_address->contact_person_name)) {
                $nameParts = explode(' ', $booking->service_address->contact_person_name);

                $customer = new \stdClass;
                $customer->first_name = isset($nameParts[0]) ? $nameParts[0] : '';
                $customer->last_name = isset($nameParts[1]) ? implode(' ', array_slice($nameParts, 1)) : '';
